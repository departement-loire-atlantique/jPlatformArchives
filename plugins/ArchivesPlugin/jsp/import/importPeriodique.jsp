<%@page import="fr.cg44.plugin.archives.importation.ImportPeriodiqueFromCsv"%>
...
if(getBooleanParameter("importPeriodique", false) && Util.notEmpty(request.getAttribute("file"))) {
    
    FileItem file = (FileItem) request.getAttribute("file");
    if (getBooleanParameter("test", false)) {
        filecheckLog = ImportPeriodiqueFromCsv.checkCsvImport(file);
    } else {
        ImportPeriodiqueFromCsv.importPeriodiquesCsv(file);
    }
}
...
<div class="jcms-message alert no-focus alert-info ">
    <h2>Notes importantes sur l'import de contenu Périodique</h2>
...
<div class="page-header"><h1>Test d'import de contenu Périodique</h1></div>

<h2>Lancer une vérification du fichier CSV sans générer d'import</h2>
<form method="POST" enctype="multipart/form-data" action="plugins/ArchivesPlugin/jsp/import/importPeriodique.jsp">
    <input type="hidden" name="importPeriodique" value="true">
